...
const getTransactionInfo = (accountId, rosettaTransaction) => {
    ...
    operations.forEach(operation => {
        ...
        const amount = (0, converter_1.parseBalance)({ value: value.toString(), decimals });
...
const getICPTransactions = async (accountId) => {
    const response = await (0, cross_fetch_1.default)(`${constants_1.ROSETTA_URL}/search/transactions`, {
        method: 'POST',
        body: JSON.stringify({
...
const getTransactions = async (localDelegationIdentity, fromAccount) => {
...
const getICPTransactionsByBlock = async (fromAccount, blockHeight) => {
    try {
        const response = await (0, cross_fetch_1.default)(`${constants_1.ROSETTA_URL}/block`, {
            method: 'POST',
            body: JSON.stringify({
                network_identifier: constants_1.NET_ID,
...
exports.getICPTransactionsByBlock = getICPTransactionsByBlock;
const getExactTransaction = (fromAccount, singleResponse, txns) => {
...
exports.getExactTransaction = getExactTransaction;
// use Rosseta API to fetch transaction list, and return
// This API works on mainnet, however not in local testnet
const getTransactionFromRosseta = (fromAccount, singleResponse, txns) => {
    const { sendArgs } = singleResponse;
    const { amount, created_at_time, memo, to } = sendArgs;
    const { blockHeight } = singleResponse;
    const { transactions } = txns;
    const found = transactions.find(val => {
        ...
        return created_at_time[0] !== undefined
            ...
            : true &&
                ...
                val.details.to === to &&
                val.details.amount ===
                    ((_a = (0, converter_1.formatAssetBySymbol)(amount.e8s, val.details.currency.symbol)) === null || _a === void 0 ? void 0 : _a.amount.toString()) &&
                val.caller === fromAccount;
    });
    return found;
};
exports.getTransactionFromRosseta = getTransactionFromRosseta;
