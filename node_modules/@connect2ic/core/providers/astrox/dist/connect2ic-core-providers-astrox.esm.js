import { _ as _classPrivateFieldGet, a as _classPrivateFieldInitSpec, b as _classPrivateFieldSet } from '../../../dist/classPrivateFieldGet-c504bd8f.esm.js';
import { _ as _defineProperty } from '../../../dist/defineProperty-fe02f37c.esm.js';
import { IC } from '@astrox/sdk-web';
import { PermissionsType } from '@astrox/connection/lib/esm/types';
import { ok, err } from 'neverthrow';
import { I as InitError, C as CreateActorError, a as ConnectError, D as DisconnectError, T as TransferError, B as BalanceError } from '../../../dist/connectors-6b4b3186.esm.js';

/* babel-plugin-inline-import '../assets/astrox_light.svg' */
// @ts-ignore
const astroXLogoLight = "data:image/svg+xml;base64,"; // @ts-ignore

/* babel-plugin-inline-import '../assets/astrox.png' */
const astroXLogoDark = "data:image/png;base64,";

var _config = /*#__PURE__*/new WeakMap();

var _identity = /*#__PURE__*/new WeakMap();

var _principal = /*#__PURE__*/new WeakMap();

var _ic = /*#__PURE__*/new WeakMap();

class AstroX {
  get principal() {
    return _classPrivateFieldGet(this, _principal);
  }

  constructor() {
    let userConfig = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};

    _defineProperty(this, "meta", {
      features: ["wallet"],
      icon: {
        light: astroXLogoLight,
        dark: astroXLogoDark
      },
      id: "astrox",
      name: "AstroX ME"
    });

    _classPrivateFieldInitSpec(this, _config, {
      writable: true,
      value: void 0
    });

    _classPrivateFieldInitSpec(this, _identity, {
      writable: true,
      value: void 0
    });

    _classPrivateFieldInitSpec(this, _principal, {
      writable: true,
      value: void 0
    });

    _classPrivateFieldInitSpec(this, _ic, {
      writable: true,
      value: void 0
    });

    _classPrivateFieldSet(this, _config, {
      whitelist: [],
      providerUrl: "https://63k2f-nyaaa-aaaah-aakla-cai.raw.ic0.app",
      ledgerCanisterId: "ryjl3-tyaaa-aaaaa-aaaba-cai",
      ledgerHost: "https://boundary.ic0.app/",
      host: window.location.origin,
      dev: true,
      noUnify: false,
      ...userConfig
    });
  }

  set config(config) {
    _classPrivateFieldSet(this, _config, { ..._classPrivateFieldGet(this, _config),
      ...config
    });
  }

  get config() {
    return _classPrivateFieldGet(this, _config);
  }

  async init() {
    try {
      var _ref;

      const ic = await IC.create({
        useFrame: !(window.innerWidth < 768),
        signerProviderUrl: "".concat(_classPrivateFieldGet(this, _config).providerUrl, "/#signer"),
        walletProviderUrl: "".concat(_classPrivateFieldGet(this, _config).providerUrl, "/#transaction"),
        identityProvider: "".concat(_classPrivateFieldGet(this, _config).providerUrl, "/#authorize"),
        permissions: [PermissionsType.identity, PermissionsType.wallet],
        ledgerCanisterId: _classPrivateFieldGet(this, _config).ledgerCanisterId,
        ledgerHost: _classPrivateFieldGet(this, _config).ledgerHost,
        dev: _classPrivateFieldGet(this, _config).dev,
        delegationTargets: _classPrivateFieldGet(this, _config).whitelist,
        noUnify: _classPrivateFieldGet(this, _config).noUnify
      });

      _classPrivateFieldSet(this, _ic, (_ref = window.ic.astrox) !== null && _ref !== void 0 ? _ref : ic);

      _classPrivateFieldSet(this, _principal, _classPrivateFieldGet(this, _ic).principal.toText()); // TODO: export Identity from @astrox/connection
      // @ts-ignore


      _classPrivateFieldSet(this, _identity, _classPrivateFieldGet(this, _ic).identity);

      const isConnected = await this.isConnected();

      if (isConnected) {
        // @ts-ignore
        _classPrivateFieldSet(this, _identity, _classPrivateFieldGet(this, _ic).identity);

        _classPrivateFieldSet(this, _principal, _classPrivateFieldGet(this, _ic).principal.toText());
      }

      return ok({
        isConnected
      });
    } catch (e) {
      console.error(e);
      return err({
        kind: InitError.InitFailed
      });
    }
  }

  async isConnected() {
    try {
      if (!_classPrivateFieldGet(this, _ic)) {
        return false;
      }

      return await _classPrivateFieldGet(this, _ic).isAuthenticated();
    } catch (e) {
      console.error(e);
      return false;
    }
  } // TODO: export & use types from astrox/connection instead of dfinity/agent


  async createActor(canisterId, idlFactory) {
    try {
      if (!_classPrivateFieldGet(this, _ic)) {
        return err({
          kind: CreateActorError.NotInitialized
        });
      } // @ts-ignore


      const actor = await _classPrivateFieldGet(this, _ic).createActor(idlFactory, canisterId); // @ts-ignore

      return ok(actor);
    } catch (e) {
      console.error(e);
      return err({
        kind: CreateActorError.CreateActorFailed
      });
    }
  }

  async connect() {
    try {
      if (!_classPrivateFieldGet(this, _ic)) {
        return err({
          kind: ConnectError.NotInitialized
        });
      }

      await _classPrivateFieldGet(this, _ic).connect({
        useFrame: !(window.innerWidth < 768),
        signerProviderUrl: "".concat(_classPrivateFieldGet(this, _config).providerUrl, "/#signer"),
        walletProviderUrl: "".concat(_classPrivateFieldGet(this, _config).providerUrl, "/#transaction"),
        identityProvider: "".concat(_classPrivateFieldGet(this, _config).providerUrl, "/#authorize"),
        permissions: [PermissionsType.identity, PermissionsType.wallet],
        ledgerCanisterId: _classPrivateFieldGet(this, _config).ledgerCanisterId,
        ledgerHost: _classPrivateFieldGet(this, _config).ledgerHost,
        delegationTargets: _classPrivateFieldGet(this, _config).whitelist,
        noUnify: _classPrivateFieldGet(this, _config).noUnify
      });

      _classPrivateFieldSet(this, _principal, _classPrivateFieldGet(this, _ic).principal.toText()); // @ts-ignore


      _classPrivateFieldSet(this, _identity, _classPrivateFieldGet(this, _ic).identity);

      return ok(true);
    } catch (e) {
      console.error(e);
      return err({
        kind: ConnectError.ConnectFailed
      });
    }
  }

  async disconnect() {
    try {
      var _classPrivateFieldGet2;

      await ((_classPrivateFieldGet2 = _classPrivateFieldGet(this, _ic)) === null || _classPrivateFieldGet2 === void 0 ? void 0 : _classPrivateFieldGet2.disconnect());
      return ok(true);
    } catch (e) {
      console.error(e);
      return err({
        kind: DisconnectError.DisconnectFailed
      });
    }
  }

  address() {
    return {
      principal: _classPrivateFieldGet(this, _principal) // accountId: this.#ic.accountId,

    };
  }

  async requestTransfer(opts) {
    var _result$payload;

    const {
      to,
      amount,
      standard = "ICP",
      symbol = "ICP",
      decimals = 8
    } = opts;

    try {
      var _classPrivateFieldGet3;

      const result = await ((_classPrivateFieldGet3 = _classPrivateFieldGet(this, _ic)) === null || _classPrivateFieldGet3 === void 0 ? void 0 : _classPrivateFieldGet3.requestTransfer({
        amount: BigInt(amount * 10 ** decimals),
        to,
        standard,
        symbol,
        // TODO: ?
        sendOpts: {}
      })); // TODO: why string? check astrox-js

      if (typeof result === "string") {
        return err({
          kind: TransferError.FaultyAddress
        });
      } // check astrox-js


      if (!result) {
        return err({
          kind: TransferError.TransferFailed
        });
      }

      switch (result === null || result === void 0 ? void 0 : result.kind) {
        case "transaction-client-success":
          return ok({
            // TODO: why is payload optional? check astrox-js
            height: Number((_result$payload = result.payload) === null || _result$payload === void 0 ? void 0 : _result$payload.blockHeight)
          });

        default:
          return err({
            kind: TransferError.TransferFailed
          });
      }
    } catch (e) {
      console.error(e);
      return err({
        kind: TransferError.TransferFailed
      });
    }
  }

  async queryBalance() {
    try {
      var _Number, _classPrivateFieldGet4;

      const ICPBalance = (_Number = Number(await ((_classPrivateFieldGet4 = _classPrivateFieldGet(this, _ic)) === null || _classPrivateFieldGet4 === void 0 ? void 0 : _classPrivateFieldGet4.queryBalance()))) !== null && _Number !== void 0 ? _Number : 0;
      return ok([{
        amount: ICPBalance / 100000000,
        canisterId: _classPrivateFieldGet(this, _config).ledgerCanisterId,
        decimals: 8,
        // TODO: plug returns image?
        // image: "Dfinity.svg",
        name: "ICP",
        symbol: "ICP"
      }]);
    } catch (e) {
      console.error(e);
      return err({
        kind: BalanceError.QueryBalanceFailed
      });
    }
  } // async signMessage({ message }: { message: string }): Promise<SignerResponseSuccess | string | undefined> {
  //   return this.#ic?.signMessage({
  //     signerProvider: this.#config.providerUrl,
  //     message,
  //   })
  // }
  // getManagementCanister: (...args) => this.#ic.getManagementCanister(...args),
  // batchTransactions: (...args) => this.#ic.batchTransactions(...args),


}

export { AstroX };
